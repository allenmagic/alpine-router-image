...
- 当前默认接口假设仍然是 `WAN=eth0`、`LAN=eth1`（`network.env` 权威，构建时替换占位符）。
- WireGuard 已移除，VPN 入口统一走 Tailscale（`ts0`/`tailscale0`）。
...
- 动态黑名单：`flood_blacklist_v4`、`flood_blacklist_v6`（超时 `1h`）
...
- 明确允许 WAN 进入的 UDP 端口：
  - `41641/udp`、`41642/udp`（`TS_PORT`），用于 Tailscale 握手
- 对自 WAN 的新建 TCP SYN 连接做限速
...
- IPv4 SYN Flood 规则排除了 `TS_PORT`，但该端口本身是 UDP 端口，因此该排除条件对 TCP 实际影响很小。
...
| VPN 接口（`ts0`/`tailscale0`） | `tcp/22` | 新建连接，`60/minute`，`burst 30` |
| `LAN` | `udp/67-68` | DHCP |
| `LAN` | `udp/53` | DNS |
| `LAN` | `tcp/53` | DNS |
...
- WAN 对本机默认只开放 Tailscale 握手（`TS_PORT`）和受限 ICMP/ICMPv6。
...
| `LAN` | `WAN` | **LAN 直连 NAT 上网**（浮动网关架构的关键路径） |
| `VPN` | `LAN` | Tailscale 访问内网 |
| `LAN` | `VPN` | 内网访问 Tailscale 侧 |

### FORWARD 链特点

- `LAN -> WAN` 直连放行是当前架构的必经路径：
  - 正常时：yunshu 容器（VRRP MASTER，持有 .254）的直连流量经上游网关（本 VM）出 WAN；
  - 降级时：容器不可用，本 VM 接管 .254，作为纯直连 NAT 保连通。
- 没有显式的 `VPN -> WAN` 放行规则：Tailscale 仅用于访问内网，不借道 WAN。
  策略分流与透明代理由 yunshu 容器负责，本 VM 只做直连 NAT。
...
1. 本机输入默认拒绝，WAN 面只开放 Tailscale 握手和受限 ICMP。
2. 内网、VPN 接口对路由器本机访问较宽松。
3. 出 WAN 的 IPv4 流量统一做 `masquerade`（LAN 直连 + VPN 出网）。
4. VPN 访问 LAN 时额外做一次 IPv4 `masquerade`，简化回程路径。
5. 转发默认拒绝，仅放行 LAN↔LAN、LAN→WAN（直连 NAT）、VPN↔LAN。
...
3. `VPN_IFS_LIST` 包含 `ts0` 和 `tailscale0`，同时又单独通过 `TS_NET` 允许 ICMP，存在“按接口”和“按地址段”混用的设计。
4. 若内网设备需要直连上网（不经 yunshu 容器），当前 `LAN -> WAN` 已放行，可直接使用；策略分流/透明代理场景请保持 DHCP 网关指向浮动 IP（.254）。
