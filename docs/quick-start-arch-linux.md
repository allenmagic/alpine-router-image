...
- 退出方式：qemu 按 `Ctrl-A X`；CH 把终端置 raw 模式（Ctrl+C 会作为字节
  发进 guest），且 guest 内 poweroff/halt 退不出 VMM（ACPI S5 无仿真）——
  需另开终端 `ch-remote --api-socket /tmp/router-vm-smoke.sock shutdown-vmm`
  （或 `pkill -f cloud-hypervisor`）
...
退出：qemu 按 `Ctrl-A X`；CH 路径另开终端 `ch-remote --api-socket /tmp/router-vm-smoke.sock shutdown-vmm`（CH 把终端置 raw 模式，Ctrl+C 只是发进 guest 的字节；guest 内 poweroff/halt 在 CH 下退不出 VMM）
